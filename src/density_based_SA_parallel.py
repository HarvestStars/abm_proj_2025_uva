...
STEPS = 50  # number of steps to run the model for each parameter set


def run_model(alpha, lambda_param, cooperation_rate, feedback_per_step_D):
    """Run the model with given parameters and return outputs."""
    model = SugarModel(
        width=10,
        height=10,
        num_agents=100,
        ...
        feedback_per_step_D=feedback_per_step_D,
    )  # conduct SA on alpha, lambda_, cooperation_rate, feedback_per_step_D
    for _ in range(STEPS):
        model.step()
    print(
        f"Model run completed with alpha={alpha}, lambda_={lambda_param}, cooperation = {cooperation_rate},feedback = {feedback_per_step_D}  {STEPS} iterations."
    )
    results = model.datacollector.get_model_vars_dataframe()
    result = results["TotalSugar"].iloc[-1]  # change the model output here if needed
...
def run_single_input(x):
    results = [run_model(*x) for _ in range(8)]
    ...
    return (
        ...
        x[3],
        avg_result,
    )  # return lambda, alpha, cooperation, feedback and result
...
def model_revaluation(input_space):
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        ...
        for i, future in enumerate(concurrent.futures.as_completed(futures)):
            alpha, lambda_param, cooperation, feedback, avg_result = future.result()
            outputs.append(avg_result)
            inputs.append([alpha, lambda_param, cooperation, feedback])
            print(
                f"Sample {i+1}: lambda={lambda_param}, alpha={alpha}, cooperation = {cooperation}, feedback = {feedback}, total_sugar={avg_result}"
            )
...
def pawn_analysis(outputs, input_space, problem):
    ...
    Si = modified_analyze(
        problem, X, Y, S=10, print_to_console=True
    )  # TODO: need to figure out how Si is calculated
...
def plot_cdf(outputs, input_space, problem):
    ...
    for index in range(problem["num_vars"]):
        ...
        fixed_vals = np.percentile(
            X[:, index], percentiles
        )  # calculate percentiple for each parameter

        for val in fixed_vals:
            # Create a mask for samples
            mask = np.abs(X[:, index] - val) < 0.05 * (
                problem["bounds"][index][1] - problem["bounds"][index][0]
            )
            Y_cond = Y[mask]
            ...
            plt.plot(
                ...
                linestyle="--",
                linewidth=2,
            )  # plot the conditional cdf for each parameter

        plt.plot(
            sorted_Y, cdf_Y, label="Unconditional CDF", color="red", linewidth=2
        )  # plot unconditional again
...
def factor_fixing(Si):
    ...
    for ks_vals in ks_all:
        fixable = all(
            k < critical_value for k in ks_vals
        )  # check the ks values and eliminate those below the critical values
...
def plot_ks_level(Si, problem, critical_value=0.15):
    ...
    for i, ks_vals in enumerate(Si["KS"]):
        ...
        plt.axhline(
            Si["median"][i], color="blue", linestyle="-", label="median KS"
        )  # plot the median
        plt.axhline(
            critical_value, color="black", linestyle="--", label="critical KS"
        )  # plot the critical level
...
if __name__ == "__main__":
    problem = {
        ...
        "bounds": [[-5, 5], [0, 200], [0.3, 0.8], [1, 4]],  # can be adjusted later
    }
    # generate parameter sample space
    input_space = hyperlatin_sample(problem, N=500)
